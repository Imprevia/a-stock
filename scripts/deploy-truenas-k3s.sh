...
for command_name in podman git curl ssh scp sha256sum helm kubectl; do
...
log "checking Traefik health through $TRUENAS_HOST:$TRUENAS_INGRESS_PORT"
curl --fail --show-error --silent \
  --max-time 15 \
...
if [[ "$SCHEDULED_COLLECTION_SUSPEND" == true ]]; then

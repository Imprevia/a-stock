...
For a new release, the ordinary entry point requires both the Helm release and any release-derived application CronJob to be absent. For an existing release, it reads the Helm stored manifest and the API-server CronJob by the exact name derived from the frozen chart render before any build, image copy/import, or write; it never treats a mutable instance-label selector's empty result as absence. Both views must contain no application CronJob, while the candidate baseline must be verified as typed `enabled=false`, `suspend=true` before its first Helm render, build, image work, SSH, or target API access. Either an active, suspended, or identity-drifted CronJob in either view blocks the ordinary operation; the operator must first freeze and review the exact baseline-plus-off packet and run the separately authorized `--disable-schedule` mode. That mode requires `rollback-v1:<approval-id>:<binding-sha256>`: the approval id is restricted text, the prefix is disjoint from Gate B/Gate C references, and the digest is computed from a versioned canonical payload containing the exact command, release, namespace, version, reviewed HEAD, chart hash, baseline hash, overlay hash, and render hash. The entry point recomputes and compares that digest before target access, so logging adjacent fields cannot masquerade as binding. Its in-flight guard starts before the Helm write; Helm failure, signals, and write-phase read/comparison failure inspect the release-derived exact API name. The emergency inspector deliberately ignores mutable labels and non-safety-critical shape drift, patches any existing resource not proven `spec.suspend=true`, then reads the same exact name back and accepts only absent or explicit boolean `true`. An unreadable, unpatchable, or unverifiable state is explicitly uncertain/NO-GO. Only a successful server-observed deletion postcondition permits a later ordinary retry. The entry point repeats the stored/exact-live check and frozen disabled-render hash immediately before the generic Helm write, which consumes the same read-only chart and values. After a successful write it verifies that the exact live application CronJob remains absent. If Helm, rollout, or any write-phase read/comparison fails, the failure guard precisely patches an unexpected active release-derived CronJob to `suspend=true`, verifies a final absent/suspended state, and still exits nonzero. If that safety postcondition cannot be proved, scheduling state remains uncertain/NO-GO. Any suspended failure outcome must pass the reviewed `--disable-schedule` workflow before an ordinary retry.

Executable deployment documentation is part of the production boundary. Its CommonMark/Bash audit therefore rejects shell constructs that can reinterpret or synthesize a Helm write, including brace/glob expansion, `eval`, stdin-fed `bash`/`sh`/`dash`/`zsh` and `source`/`.` paths, command-resolution mutation through `alias` or `hash -p`, dynamic executables/actions, and unknown Helm plugins/actions. Parse ambiguity is a violation rather than permission to skip an action.
...
Gate A was approved on 2026-09-08 (Asia/Shanghai) in `GYT-45`. The approval evidence is member comment `01a07e3a-141c-71ac-b30b-0f06cf0c4a8b`, followed by the recorded gate decision in `01a07e3d-5a77-7972-a9cd-360c9d41dc84`. It authorizes repository implementation and offline verification only. Gate B/Gate C progression authorization was later recorded in `GYT-47` comment `01a07fcd-9140-7aa7-b05a-83485a7ed8c7`. GYT-52 rejected exact `origin/main=0b319c1501d6706f0be4eb680c46dd0d66f2c4dc`: rollback authorization was not canonically bound, active-to-off recovery was not robust to label/shape drift, documentation command auditing missed stdin/source/alias paths, and ordinary deploy did not strictly reject disabled-but-unsuspended values. Historical NO-GO is retained as remediation history. GYT-47 implementation parent `cd26dff3e6bebe012198dcd38074c354c1a9afac` closes those four gaps on a clean successor that excludes GYT-21; `b13ed06fb729cc3a2c52908ba45136717bf18bed`, `7d74b8b`, `0717d840d8c2777bef666a208d28ebe4288c77c1`, and frozen review/evidence tip `4f6d2b28b1694c78f53eb3ce007b8530b0667ead` are docs-only wrappers and do not alter implementation or test scope. Implementation tests are bound to `cd26dff`; clean/review/evidence claims are bound to the frozen tip `4f6d2b2`. GYT-52 subsequently gave Gate A GO on exact clean `origin/main=main=HEAD=b1907e63ab81ca9c5e1d9d5531aadbf3118f2998` (comment `01a08af1-2842-7c07-80a6-2af49b91e196`). Production execution remains blocked until exact Gate B packet review and authorization, Gate B evidence acceptance, and the later Gate C catch-up decision.
...
The shared worktree is not an implementation baseline because it contains unrelated tracked and untracked changes. GYT-47 therefore uses isolated branch `agent/backend/gyt-47-gate-a-remediation-v2`, rooted at exact clean `0b319c1501d6706f0be4eb680c46dd0d66f2c4dc`; that base descends from `b6d3942a7cf75d51b9c6efebcef71b7317931fb6` and excludes GYT-21 commit `99a625a38bff8bdbfc421fb21176565b8c9d3028`. Only the six scheduling-machine-file deltas from the rejected remediation were mechanically replayed; planning and documentation safety hunks were manually reconciled against the current facts. The current scope contains no `src/`, `apps/`, `trading-rules/`, GYT-21 service/test, or unrelated completed-plan changes. A new clean candidate, fresh offline evidence, and GYT-52 verdict must bind to this lineage. If implementation requires a new external configuration/API contract not stated in these approved artifacts, work stops and returns for architecture review.
...
| M2 - Repository implementation | D2-D3 | Senior backend engineer (`GYT-47`) | M1 passes | Tasks 2.1-2.8: Helm strategies/validation, three TrueNAS overlays, deployment entry point, Kustomize boundary, generic release lifecycle guard, canonical rollback authorization and exact-name recovery; focused tests green | Remediation complete at exact `cd26dff3e6bebe012198dcd38074c354c1a9afac` |
| M3 - Offline verification and review packet | D4 | Senior backend engineer (`GYT-47`, then independent `GYT-52`) | Remediated `GYT-47` candidate | Tasks 3.1-3.8: implementation tests on `cd26dff`; Helm/OpenSpec/docs/diff and clean review evidence on frozen tip `4f6d2b2` | Fresh offline gates complete; exact candidate accepted by GYT-52 Gate A GO |
| M4 - Production validation preparation | Unscheduled | Unassigned until prerequisites pass | GYT-52 exact-main GO plus exact packet review | Tasks 4.x-5.x evidence packet and bounded production validation | Gate A GO recorded; blocked on target access, packet review, and action authorization |
...
M1 and M2 are one backend implementation stage because the documentation baseline had to be completed before its first code edit. M2 remediation and replacement M3 evidence are complete on the clean successor: implementation/tests at `cd26dff`, docs-only review/evidence tip at frozen `4f6d2b2`. GYT-52 has completed its independent Gate A verdict with GO on exact `b1907e63ab81ca9c5e1d9d5531aadbf3118f2998`; Tasks 4.x onward remain blocked until the exact Gate B packet is reviewed and action-authorized.
...
2. M1 remains complete, but GYT-52's historical NO-GO reopened M2 task 2.8 and required the current clean-lineage remediation; no historical exact-SHA GO transfers to the successor candidate.
3. The prior M3 packet was invalidated. Fresh tasks 3.3-3.8 evidence is bound to the new candidate and was accepted by GYT-52 Stage 4 Gate A GO.

...
- Make the Chart and every generic install, upgrade, or application rollback fail closed with scheduled collection disabled and suspended; route all documented production writes through `scripts/deploy-truenas-k3s.sh`, do not inherit opaque release values or restore historical Helm values outside the guarded Gate B/Gate C path.
- Before a generic build or target write, require the Helm stored manifest and live state to contain no application CronJob. An active or suspended CronJob in either view must first be removed through a separately reviewed and authorized `--disable-schedule` operation. A generic write succeeds only after the server-observed schedule remains absent; on failure, an unexpected active CronJob is precisely suspended and verified as absent/suspended, while an unreadable or uncertain postcondition remains a failure and NO-GO.
...
- Gate B and Gate C progression authorization was recorded later in `GYT-47` member comment `01a07fcd-9140-7aa7-b05a-83485a7ed8c7`. Execution remains blocked: after earlier remediation, Stage 3 completed from exact clean baseline `dd1277b86d04ed3a5d2cabe41c558d6de9049c09`, but independent test GYT-52 rejected Stage 3 HEAD `b231ef4507e4003d2a5d3fe3a25ec1d659d7cb75` because generic Helm defaults and documented install/upgrade/rollback paths could bypass Gate B/Gate C. The first successor `9478ff0fd946993ae582b75dbb3287cf3a818aea` also received independent NO-GO because raw generic Helm writes could atomically restore an old active revision, stored/live schedule was not a precondition, archived guidance retained an executable historical rollback, and the command audit had parsing gaps. GYT-47 remains reopened for the second successor, including the stored/live-off precondition and post-write schedule check; GYT-48/GYT-52 are backlogged until a new clean HEAD and offline evidence are accepted. The exact Gate B packet must still be reviewed, and Gate C must explicitly select `next-schedule` or `immediate catch-up` after Gate B evidence is accepted.

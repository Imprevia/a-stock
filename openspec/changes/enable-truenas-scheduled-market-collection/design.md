...
| M3 - Offline verification and review packet | D4 | Senior backend engineer (`GYT-47`, then independent `GYT-52`) | Remediated `GYT-47` candidate | Tasks 3.1-3.8: render matrix, manifest invariants, fake-provider regressions, Helm/OpenSpec/docs/diff gates, command audit and release lifecycle adversarial negatives | Fresh offline gates complete; exact candidate submitted for independent review |
...
M1 and M2 are one backend implementation stage because the documentation baseline had to be completed before its first code edit. M2 remediation and replacement M3 evidence are complete on the clean successor. The remaining Gate A checkpoint is GYT-52's independent verdict on the pushed exact HEAD. Tasks 4.x onward are not assigned in this Gate A plan.

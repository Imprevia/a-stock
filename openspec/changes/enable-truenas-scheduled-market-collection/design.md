...
Gate A was approved on 2026-09-08 (Asia/Shanghai) in `GYT-45`. The approval evidence is member comment `01a07e3a-141c-71ac-b30b-0f06cf0c4a8b`, followed by the recorded gate decision in `01a07e3d-5a77-7972-a9cd-360c9d41dc84`. It authorizes repository implementation and offline verification only. Gate B/Gate C progression authorization was later recorded in `GYT-47` comment `01a07fcd-9140-7aa7-b05a-83485a7ed8c7`. Stage 4 rejected historical HEAD `b231ef4507e4003d2a5d3fe3a25ec1d659d7cb75`; the subsequent remediation candidate `0cd9b31fc3f3a35dc404f6bafe8af8f87dc66ca2` received three independent GO verdicts. The approved GYT-47 remediation and completed GYT-48 delivery have now been replayed onto `origin/main=b6d3942a7cf75d51b9c6efebcef71b7317931fb6`, excluding GYT-21. Focused, fake-provider and render gates pass; an exact-main full rerun records two failures in baseline-identical market-service tests, including the known cold-load TOCTOU, without retrying or changing their assertions. GYT-52 must independently revalidate the final pushed main SHA and assess those residuals. Production execution remains blocked until that GO, exact Gate B packet review and authorization, Gate B evidence acceptance, and the later Gate C catch-up decision.
...
The shared worktree is not an implementation baseline because it contains unrelated tracked and untracked changes. GYT-47 therefore used isolated branches/worktrees rooted in recorded clean baselines. For local-main-first closeout, a new isolated integration worktree replayed exactly the 23 commits in `99a625a..4a2a617` onto the latest remote-main baseline; the two active-plan index conflict resolutions retained the remote state and omitted GYT-21's plan row. The resulting diff has no `src/`, `apps/`, `trading-rules/`, GYT-21 service/test, or unrelated completed-plan changes. Because replay changes commit identities, historical exact-SHA verdicts remain evidence for the source patches but GYT-52 must bind its verdict to the final main SHA. If implementation requires a new external configuration/API contract not stated in these approved artifacts, work stops and returns for architecture review.
...
| M2 - Repository implementation | D2-D3 | Senior backend engineer (`GYT-47`) | M1 passes | Tasks 2.1-2.7: Helm strategies/validation, three TrueNAS overlays, deployment entry point, Kustomize boundary, generic release lifecycle guard; focused tests green | Approved range integrated into local main; exact-main revalidation pending |
| M3 - Offline verification and review packet | D4 | Senior backend engineer (`GYT-48`) | `GYT-47` reviewed | Tasks 3.1-3.7: render matrix, manifest invariants, fake-provider regressions, Helm/OpenSpec/docs/diff gates, command audit and generic lifecycle negatives | Accepted delivery integrated; rebased gates pass |
...
M1 and M2 are one backend implementation stage because the documentation baseline had to be completed before its first code edit. M3 is completed and its four approved commits are included in the rebased local-main candidate. The remaining Gate A checkpoint is GYT-52's independent verdict on the final pushed main SHA. Tasks 4.x onward are not assigned in this Gate A plan.
...
2. M1/M2 historically completed on an isolated reviewed clean worktree; Stage 4 rejected the first Stage 3 aggregate, and remediated M2 exact candidate `0cd9b31fc3f3a35dc404f6bafe8af8f87dc66ca2` later received three independent GO verdicts. Its approved patch range is now integrated on the latest main baseline.
3. M3 is complete and integrated. GYT-52 must rerun Stage 4 against the final pushed main SHA because exact-SHA approval does not transfer across replayed commits.

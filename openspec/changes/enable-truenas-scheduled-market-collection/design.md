...
The Chart itself also defaults to `enabled=false` and `suspend=true`. Generic Dashboard installs, upgrades, and application rollbacks must re-submit a complete version-controlled environment values file plus those two explicit safety overrides. They must not inherit stored release values or replay a historical Helm revision, because either path can carry an unreviewed `enabled=true`, `suspend=false` state around the Gate B/Gate C entry point. An application rollback is therefore a new atomic upgrade from a reviewed rollback commit/chart, complete environment values, the previous immutable image tag, and the disabled scheduling state. Only the guarded scheduling entry point may consume the scheduled overlays.
...
| M2 - Repository implementation | D2-D3 | Senior backend engineer (`GYT-47`) | M1 passes | Tasks 2.1-2.5: Helm strategies/validation, three TrueNAS overlays, deployment entry point, Kustomize boundary; focused tests green | Accepted / terminal |
| M3 - Offline verification and review packet | D4 | Senior backend engineer (`GYT-48`) | `GYT-47` terminal and reviewed | Tasks 3.1-3.5: render matrix, manifest invariants, fake-provider regressions, Helm/OpenSpec/docs/diff gates, clean reviewable diff | Complete; awaiting acceptance |

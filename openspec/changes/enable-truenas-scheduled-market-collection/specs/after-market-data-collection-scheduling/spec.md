...
- **THEN** the operation uses complete reviewed environment values with `enabled=false` and `suspend=true`, does not inherit stored release values, and cannot create or restore an active scheduled-collection CronJob

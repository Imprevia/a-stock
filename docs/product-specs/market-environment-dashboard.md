...
- 无 scheduling overlay 的默认 Helm render 不包含 CronJob；所有文档化通用 Helm 写命令显式提交完整环境 values 与 `enabled=false`、`suspend=true`，不得绕过 Gate B/Gate C 恢复 active schedule。

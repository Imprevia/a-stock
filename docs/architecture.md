...
> 状态：**已落地第 01 章市场环境看板与交易规则工程平台首期**。产品范围见 `docs/product-specs/market-environment-dashboard.md` 与 `docs/product-specs/trading-rule-engineering.md`。
...
市场环境 API 通过可选的 `chapter01` 对象扩展第 01 章证据：全 A 当前快照用于上涨/下跌家数、涨跌幅中位数和成交额前列股票；东方财富日期化涨停、跌停和炸板池用于打板生态；行业板块排名用于当日方向线索。只有所选日期对应当前最新市场快照时才允许使用全 A 与板块当前排名，历史日期不会复用今日数据。暂未接入的高/中/低位亏钱效应和事件输入保持 `null` / `insufficient`，并附 provider quality 和 warning。

网页使用固定一级导航 `如何判断市场环境`，下设 01 至 09 文档视图。每个视图只解释 API 已返回的证据与质量状态；前端不补算缺失指标，不把未验证阈值渲染为确定性评分。移动端将同一导航收纳为抽屉，宽表保留横向滚动。

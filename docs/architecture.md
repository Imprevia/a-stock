...
### `breadth` / `activeDirection` 通达信盘后包备用边界（2026-09-23）
...
盘后包的日期、证券代码、名称、收盘价、前收、成交额和市场完整性均需通过校验；缺失事实保持 `null` 或拒绝候选，不能用零值、代码字符串、当前行情或其他日期回填。`breadth` 优先使用包内涨跌幅/前收，缺失时只允许通过精确交易日历取得相邻上一交易日包并校验身份覆盖率。`activeDirection` 只接受包的原始成交额非递增顺序，禁止本地排序任意全市场行；名称优先使用包内名称，缺失时仅对 Top-N 批量请求腾讯名称，腾讯不得提供价格、成交额或日期事实。开关 `MARKET_ENVIRONMENT_TDX_DAILY_PACKAGE_FALLBACK_ENABLED` 默认关闭，质量来源为 `tdx-daily-package` 时必须保留前序东方财富 warning；包未发布、日期冲突、格式损坏或字段不足仍为 `failed`/`insufficient`，由 collection 层继续执行 `failed-missing`/`failed-retained`。

...
```text
浏览器 → Vite `/api` 代理 → `/api/market-environment/core` → 指数 provider → 首个可用视图
                         └→ `/api/market-environment/chapter-01` → 扩展 provider → 渐进补齐章节证据
已有调用方 ───────────────→ `/api/market-environment` → 聚合上述两层并保持原响应契约
```

服务层按请求日期分别缓存核心指数、章节证据和兼容聚合响应 30 秒。章节层复用同日期的核心缓存计算组合汇总，不重复抓取指数；前端只在两层 `asOf` 一致时合并结果。扩展 provider 变慢或失败只影响章节层，不能阻塞或清空已经返回的核心指数。该边界用于隔离东方财富共享串行 limiter 下的全 A、行业和涨跌停池延迟；历史实测市场广度 fallback 单独约 12 秒，因此首屏不再等待整章证据完成。

核心层至少请求 120 个交易日，先按 `as_of` 截断到最近交易日，再计算 MA5/10/20/60、20/60 日高低价区间位置、成交额比值、趋势状态和量价状态。量价状态只在明确命中价格变化与 5 日成交额比值规则时返回；“量价平稳”限定为日涨跌幅绝对值小于 `0.5%` 且比值位于 `[1.0, 1.2)`，其他阈值空档返回 `null`，禁止通用兜底分类。服务层进一步按量化版 `0.2` 的风险优先映射计算每个指数的六类位置量价组合、触发证据和交易模式；章节层结合五大指数同步性与可用市场广度生成市场强弱、阶段、资金认可和交易模式四项汇总。前端只消费这些结果，不重复计算阈值。返回给前端的 60 日历史点保留真实 `open`、`high`、`low`、`close`、成交额和均线；前端仅负责将 OHLC 渲染为 K 线，不合成或补算行情。单指数失败保留其他指数并写入 warning；全部失败返回 503。
...
市场环境 API 保留原有聚合 `indices` / `summary` / 可选 `chapter01` 契约，同时提供核心与章节两个窄接口用于渐进加载。任何 provider 缺失均使用 `null`、`partial`、`missing` 或 `insufficient` 表达；只有上游明确返回空池时才可将对应计数记为 0，分母为 0 的比率仍为 `null`。规则平台继续通过独立 snapshot 契约执行确定性评分。

...
- 2026-09-22 完成 TrueNAS k3s 受控发布：retained CronJob 已通过 `--disable-schedule` 删除并证明 absent；普通 `--component all` 构建并导入镜像 `localhost/a-stock-market-environment:20260922-185527-06c3fd7`，Helm revision 33 deployed，最终 stored `component=all` 且调度 disabled/absent。
